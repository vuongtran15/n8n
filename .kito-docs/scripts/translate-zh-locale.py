#!/usr/bin/env python3
"""Machine-translate remaining en fallback strings in zh.json. Resume-safe."""
...
LOG_PATH = Path(__file__).resolve().parent / "translate-zh.log"
...
def main() -> int:
    ...
    for path, en_val in walk_leaves(en):
        try:
            zh_val = get_by_path(zh, path)
        except KeyError:
            zh_val = None
        if zh_val is None or zh_val == en_val:
    ...
    while i < len(pending):
        ...
        if batch_texts:
            ...
            for path, result, mapping in zip(batch_paths, results, batch_maps, strict=True):
                ...
                set_by_path(zh, path, translated)
