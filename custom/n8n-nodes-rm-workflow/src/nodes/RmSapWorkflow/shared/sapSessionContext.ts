...
import { coalesceNumber, valueFromItemJson, valueFromItemJsonAliases } from './sapItemJson';
import { normalizeBaseUrl, normalizeRequestUrl } from './sapApi';
...
const DEFAULT_WECOM_TIMEOUT_SECONDS = 30;
...
/** Chỉ timeout tùy chọn từ input JSON; không yêu cầu baseUrl / apiKey / sessionId. */
export function resolveWecomExecutionContext(
	...
	const v = valueFromItemJson(itemJson, 'requestTimeoutSeconds');
...
export interface SapApiConnectionContext {
	itemJson: IDataObject;
	useInputJsonFields: boolean;
	baseUrl: string;
	apiKey: string;
...
export interface SapSessionConnectionContext extends SapApiConnectionContext {
	sessionId: string;
}

function resolveSapBaseConnectionFields(
	ctx: IExecuteFunctions,
	itemIndex: number,
): SapApiConnectionContext {
	const items = ctx.getInputData();
	const itemJson = items[itemIndex]?.json ?? {};
	const useInputJsonFields = true;
	...
	const timeoutSeconds = optionalInputNumber(
		...
		DEFAULT_SAP_REQUEST_TIMEOUT_SECONDS,
	);
...
/** Base URL + API key — không cần sessionId (route quản trị session). */
export function resolveSapApiConnectionFields(
	ctx: IExecuteFunctions,
	itemIndex: number,
): SapApiConnectionContext {
	return resolveSapBaseConnectionFields(ctx, itemIndex);
}

/**
 * Base URL, API key, sessionId — bắt buộc từ input JSON.
 * requestTimeoutSeconds tùy chọn (mặc định 10 phút).
 */
export function resolveSapSessionConnectionFields(
	ctx: IExecuteFunctions,
	itemIndex: number,
): SapSessionConnectionContext {
	const base = resolveSapBaseConnectionFields(ctx, itemIndex);
...
export function apiConnectionPassThrough(conn: SapApiConnectionContext): IDataObject {
...
export function connectionPassThrough(conn: SapSessionConnectionContext): IDataObject {
	const out: IDataObject = {
		baseUrl: conn.baseUrl,
		apiKey: conn.apiKey,
